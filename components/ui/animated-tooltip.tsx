 ...
 * Inspired by Codrops' "Tooltip Animations" (originally anime.js).
 *
 * From the VengeanceUI registry, with the theming moved onto this project's
 * tokens. The original defaulted its bubble to `var(--foreground)` over
 * `var(--background)` - shadcn variables that do not exist here - so the
 * defaults below are the mint fill with ink type, which is the pairing the
 * palette notes in globals.css call for (mint carries dark text, never
 * white). Imports come from motion/react rather than a second copy of
 * framer-motion.
 */
...
export interface AnimatedTooltipProps {
  ...
  /** Trigger text color while the tooltip is open. Defaults to the mint accent. */
  accentColor?: string;
  /** Trigger text color at rest. Defaults to body text, so it sits in a paragraph. */
  restColor?: string;
  /** Bubble fill color. Defaults to the mint accent. */
  shapeColor?: string;
  /** Tooltip text color. Defaults to ink, which is what mint fills carry. */
  textColor?: string;
...
export function AnimatedTooltip({
  ...
  shapeColor = "var(--color-signal)",
  textColor = "var(--color-ink-950)",
  className,
