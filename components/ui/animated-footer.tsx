...
function buildHandCells(
  ...
  /*
    getImageData throws a SecurityError on a tainted canvas. The images are
    same-origin so this should not fire, but a future caller pointing
    leftImage at a CDN without CORS headers would otherwise take the whole
    footer - headings and all - down with it. Failing to an empty cell map
    degrades to "no ASCII art", which is survivable.
  */
...
export function AnimatedFooter({
  ...
  leftImage = "/animated-footer/funnel-left.svg",
  rightImage = "/animated-footer/funnel-right.svg",
  background,
  textColor,
  charColor,
  hoverColor,
  ...
  /*
    The registry version resolved these against next-themes and shipped an
    orange pair. This site has no theme switch - it is dark by construction,
    and globals.css says so - so the hook was a dependency that could only
    ever return one answer. The defaults are the palette's mint instead:
    recessive glyphs that light to full signal under the pointer, carrying
    ink type, which is the contrast direction the palette notes require.
  */
  ...
  /*
    A div, not a <footer>. The registry version is a <footer>, but this band
    is decorative - it holds no navigation, no contact details and no legal
    links - and it is composed inside the site's real <footer>, where a
    nested footer element is invalid: the content model for footer forbids
    header and footer descendants. The landmark stays with the consumer.
  */
  return (
    <div
      ref={rootRef}
